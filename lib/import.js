...
const importLines = async (task, lines, model, totalLineCount) => {
  ...
  const fieldNames = model.schema.map(column => column.name).filter(name=>name !== 'id'); //id doesnt exist in gtfs. autoincrement field
  const placeholders = [];
  const values = [];
...
module.exports = async config => {
  ...
  await Promise.mapSeries(config.agencies, async agency => {
    // agency key no longer necessary
    ...
    cleanup();
    task.log('Completed GTFS import');//should i do client.end()??? or db.client.end? whatever
  });
